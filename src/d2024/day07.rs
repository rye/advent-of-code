...
fn part_one() {
	...
	let intermediate = solver.parse(include_str!("day07.example.in.txt")).unwrap();

	assert_eq!(Some("3749".to_string()), solver.part_one(&intermediate));
}
...
fn part_two() {
	...
	assert_eq!(Some("11387".to_string()), solver.part_two(&intermediate));
}
